...
class add_estrutura_course{

    protected static $metabox_config = array(
        'add_estrutura_course',
        'Estrutura do Curso',
        'course',
        'normal'
    );
    ...
    static function metabox(){
        ...
        if(!isset($metadata['estrutura_course'])){
           $content = "";
        }else{
            $content = $metadata['estrutura_course'][0];
        };
        ?>
        <textarea name="<?php echo __CLASS__ ?>[estrutura_course]" style="background-color: #fff; font-size: 1.7em; height: 4em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;">
        	<?php echo $content; ?>
        </textarea>
        <?php
    }
