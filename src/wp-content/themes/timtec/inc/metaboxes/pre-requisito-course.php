...
class add_pre_requisito_course{

    protected static $metabox_config = array(
        'add_pre_requisito_course',
        'Pré-Requisitos',
        'course',
        'normal'
    );
    ...
    static function metabox(){
        ...
        if(!isset($metadata['pre_requisito_course'])){
           $content = "";
        }else{
            $content = $metadata['pre_requisito_course'][0];
        };
        ?>
        <textarea name="<?php echo __CLASS__ ?>[pre_requisito_course]" style="background-color: #fff; font-size: 1.7em; height: 4em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;">
        	<?php echo $content; ?>
        </textarea>
        <?php
    }
