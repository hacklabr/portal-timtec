...
class add_url_youtube_course{

    protected static $metabox_config = array(
        'add_url_youtube_course',
    ...
    static function metabox(){
        ...
        if(!isset($metadata['url_youtube_course'])){
           $link = "";
        }else{
            $link = $metadata['url_youtube_course'][0];
        };
        ?>
        <input type="text" name="<?php echo __CLASS__ ?>[url_youtube_course]" value="<?php echo  $link; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;" />
        <?php
    }
