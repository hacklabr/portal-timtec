...
class add_qtd_aulas_course{

    protected static $metabox_config = array(
        'add_qtd_aulas_course',
        'Quantidade de aulas',
        'course',
        'side'
    );
    ...
    static function metabox(){
        ...
        if(!isset($metadata['qtd_aulas_course'])){
           $content = "";
        }else{
            $content = $metadata['qtd_aulas_course'][0];
        };
        ?>
        <input type="number" name="<?php echo __CLASS__ ?>[qtd_aulas_course]" value="<?php echo $content; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;" />
        <?php
    }
