...
class add_link_metabox_redesocial {
    ...
    static function metabox(){
        ...
        $metadata = get_metadata('post', $post->ID);
         if(!empty($metada)){
            $metadata['rede_social_link'][0];
         }else{
            $link = "";
         };

        ?>
        <input type="text" name="<?php echo __CLASS__ ?>[rede_social_link]" value="<?php echo $link; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;"/>
        <?php
    }
...
add_link_metabox_redesocial::init();
